export class FormatText {
    ...
    private static readonly nonASCII = /[^\x00-\x7F]/gm;
    private static readonly firstWhiteSpace = /^ /;
    private static readonly lastWhiteSpace = / $/;
    ...
    public static normalizeText(source: string) {
        const sanitized = source.normalize("NFD").replace(FormatText.accentsRgx, "");
        const normalized = sanitized
            .replaceAll(FormatText.specialCharsRgx, "")
            .replaceAll(FormatText.doubleDashesRgx, "-")
            .replaceAll(FormatText.nonASCII, "")
            .replace(FormatText.lastWhiteSpace, "");
        return normalized;
    }

    public static toSearchParam(source: string, separator: string) {
        let res = FormatText.normalizeText(source).toLocaleLowerCase();
        let param = res.replaceAll(FormatText.whiteSpacesRgx, separator);
        return param;
    }

    public static removeLateralWhiteSpaces(source: string) {
        return source
            .replace(FormatText.firstWhiteSpace, "")
            .replace(FormatText.lastWhiteSpace, "");
    }
