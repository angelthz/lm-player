export class TimeUtils {
    ...
    public static formatMM(time: number) {
        return TimeUtils.toMins(time) < 10 ? `0${TimeUtils.toMins(time)}` : `${TimeUtils.toMins(time)}`;
    }

    public static formatSS(time: number) {
        return TimeUtils.toSecs(time) < 10 ? `0${TimeUtils.toSecs(time)}` : `${TimeUtils.toSecs(time)}`;
    }

    public static formatCC(time: number) {
        return TimeUtils.toCens(time) === 0 ? "00" : TimeUtils.toCens(time).toFixed(2).replaceAll(/0|\./g, "");
    }
    ...
    */
    public static toFullTMS(time: number) {
        return `${TimeUtils.formatMM(time)}:${TimeUtils.formatSS(time)}.${TimeUtils.formatCC(time)}`;
    }
    ...
    */
    public static toShortTMS(time: number) {
        return `${TimeUtils.formatMM(time)}:${TimeUtils.formatSS(time)}`;
    }
    ...
    */
    public static parseMM(tms: string) {
        if (TimeUtils.isValidTMS(tms)) {
            let secs = TimeUtils.parse(tms);
            return TimeUtils.toMins(secs);
        }
        else
            return -1;
    }
